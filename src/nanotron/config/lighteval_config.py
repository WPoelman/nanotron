...
class LightEvalConfig:
    ...
    eval_interval_file: Optional[Path] = (
    ...
    def __post_init__(self):
        ...
        if self.upload_to_wandb:
            assert self.s3_save_path is not None, (
                " We should have a s3_save_path if we want to upload to wandb"
            )  # todo: add the option to read from local folder i guess
